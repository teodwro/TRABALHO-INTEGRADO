...
final class EmpresaModel extends Model {
    ...
    ON e.representante_empresa = r.id_representante;";

        $data = $db->select($query);
        $arrayList = [];

        foreach ($data as $row) {
            $vo = new EmpresaVO($row['id_empresa'], $row['nome_empresa'], $row['endereco_empresa'], $row['telefone_empresa'], $row['email_empresa'], $row['cnpj_empresa'], $row['nome_representante']);
